...
- 自下而上：背景 → **整幅高斯虚化**（全画布同一模糊，超采样后缩回）→ **均匀黑色遮罩**（整幅同一 alpha，默认 **0.4×255**）→ 纯白标题 → 五张底栏图。
- 底部**固定 5 张**竖条：`2..9` 从左到右取用，不足 5 张则循环补齐；无 `2..` 时五格均用背景图。
- Emby 拉取：`1.jpg` 为媒体[0] 的 Fanart 链；`2～6.jpg` 为媒体[1]～[5] 的 Primary（见 `cover_emby_fetch`）。
- 左上标题：中英字号比例与 **style_single_1/2** 一致（约 0.17H / 0.07H × font_size）；中英行距为原逻辑约 **2 倍**；`margin_top` 由块高与 `row_y` 自动重算居中。
"""
...
# 标题字号与 style_single_1 / style_single_2 一致（相对画布高）
_SHELF_ZH_H_RATIO = 0.17
_SHELF_EN_H_RATIO = 0.07
_SHELF_ZH_SIZE_MIN = 52
_SHELF_EN_SIZE_MIN = 24
# 中英行距：相对原猜你喜欢/旧 shelf 约加倍
_SHELF_ZH_EN_GAP_MIN = 44
...
def create_style_shelf_1(
    ...
    try:
        ...
        canvas = _full_canvas_gaussian_blur(canvas, blur_radius=36, scale=3)
        ...
        # 整幅均匀黑色遮罩；标题与底栏在遮罩之上
        canvas = Image.alpha_composite(canvas, _black_vertical_mask((w, h), max_alpha_ratio=0.4))
