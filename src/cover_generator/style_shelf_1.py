...
- 九槽各拉 **Primary**（`n.*`）与宽屏背景（`fanart_n.*`，Emby 上为 **Fanart 或 Backdrop**，见 `cover_emby_fetch`）。**先**在已有 `fanart_*` 里按复杂度 **选出全屏底图**；**再**对其余 **8 槽** 按 **1→9** 用 **Primary** 顺序去重填 **5** 格，不足用当前底图文件补齐。
...
def _shelf_bg_complexity_score(path: Path) -> float:
    """
    越低越适合作虚化全屏背景：FIND_EDGES 能量（细节/主体轮廓少）与 RGB 通道 std（色彩块不过花）加权。
    缩放到长边约 240px 再算，控制耗时。
    """
    try:
    ...
    if max(w0, h0) > max_side:
        ...
        nw = max(2, int(w0 * scale))
        nh = max(2, int(h0 * scale))
...
def create_style_shelf_1(
    ...
    """
    :param library_dir: 九槽各 `n.*`（Primary）与 `fanart_n.*`（Fanart）；须至少一张 Fanart 以评选底图；底栏五格由其余槽 Primary 顺序去重得到。
    ...
    try:
        ...
        if not fanart_pool:
            logger.error(
                "style_shelf_1: no fanart_1…9.* (Fanart-only background pool) in %s",
                library_dir,
            )
            return False
