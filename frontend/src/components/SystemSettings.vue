    ...
    <el-form label-position="top" label-width="100px" v-if="store.config" class="settings-form">
      ...
      <div class="form-section">
        ...
        <template v-if="store.config.webhook && store.config.webhook.enabled">
          <div class="form-row two-col">
            <el-form-item label="回调 URL（POST）" class="form-item-flex">
              <el-input :model-value="webhookCallbackUrl" readonly />
              <div class="form-item-description" v-if="store.config.webhook.secret">
                使用密钥时也可：<code>{{ webhookCallbackUrl }}?token=你的密钥</code>
              </div>
            </el-form-item>
