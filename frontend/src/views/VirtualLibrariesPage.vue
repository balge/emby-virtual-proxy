  ...
  <div>
    ...
    >
      ...
      >
        ...
        <BaseButton
          class="min-w-0"
          @click="store.fetchAllEmbyData"
          :loading="store.dataLoading"
          >刷新Emby数据</BaseButton
        >
        <BaseButton class="min-w-0" @click="store.openLayoutManager"
          >调整布局</BaseButton
        >
        <BaseButton
          class="min-w-0"
    ...
    <div v-else-if="!store.virtualLibraries.length" class="text-center py-20">
      ...
      <BaseButton variant="primary" class="mt-4" @click="store.openAddDialog"
        >创建第一个虚拟库</BaseButton
      >
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          ...
          >
            ...
            <th
              class="text-center px-4 py-3 font-medium text-gray-500 dark:text-gray-400 w-44"
            >
              封面
            </th>
            <th
              class="text-left px-4 py-3 font-medium text-gray-500 dark:text-gray-400 w-20"
            >
              类型
            </th>
            <th
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
          ...
          >
            <td class="px-4 py-3">
              <div class="flex items-center gap-2 min-w-0">
                <template v-if="rowSyncing(row.id)">
                  <span
                    class="inline-flex shrink-0 text-primary-600 dark:text-primary-400"
                    aria-hidden="true"
                  >
                    <svg
            ...
            <td class="px-4 py-3">
              <BaseTag :variant="typeTagVariant(row.resource_type)">{{
            ...
            <td class="px-4 py-3">
              <div class="space-y-1">
                <p
                  ...
                  :title="getResourceDetail(row)"
                >
                  {{ getResourceDetail(row) }}
                </p>
                <div class="flex flex-wrap gap-1">
                  <BaseTag v-if="row.merge_by_tmdb_id" variant="success"
                    >TMDB合并</BaseTag
                  >
                  <BaseTooltip
                    v-if="row.source_libraries?.length"
                    :text="getSourceLibNames(row)"
                  >
                    <BaseTag variant="warning"
                      >源库: {{ getSourceLibSummary(row) }}</BaseTag
                    >
                  </BaseTooltip>
                  <BaseTag
                    v-else-if="row.resource_type !== 'rsshub'"
                    variant="default"
                    >全部源库</BaseTag
                  >
                  <BaseTag v-if="row.advanced_filter_id" variant="info">{{
                    getFilterName(row.advanced_filter_id)
                  }}</BaseTag>
                  <BaseTag v-if="getRefreshText(row)" variant="default">{{
                    getRefreshText(row)
                  }}</BaseTag>
                </div>
                <p
                  v-if="row.resource_type === 'rsshub' && row.rsshub_url"
                  class="text-[11px] text-gray-400 dark:text-gray-500 break-all font-mono leading-relaxed"
                >
                  {{ row.rsshub_url }}
                </p>
              </div>
            ...
            <td class="px-4 py-3">
              <div class="flex justify-end gap-1 flex-wrap">
                <BaseTooltip
                  v-if="row.resource_type === 'rsshub'"
                  text="刷新 RSS"
                >
                  <BaseButton
                    size="xs"
                    variant="success-outline"
                    @click="store.refreshRssLibrary(row.id)"
                    ><RssIcon class="w-3.5 h-3.5" /> RSS</BaseButton
                  >
                </BaseTooltip>
                <BaseTooltip text="刷新封面">
                  <BaseButton
                    size="xs"
                    @click="store.refreshLibraryCover(row.id)"
                    ><ArrowPathIcon class="w-3.5 h-3.5" /> 封面</BaseButton
                  >
                </BaseTooltip>
                <BaseTooltip
                  v-if="row.resource_type !== 'rsshub'"
                ...
                >
                  <BaseButton
                    size="xs"
                    variant="success-outline"
                    @click="store.refreshLibraryData(row.id)"
                    ><CircleStackIcon class="w-3.5 h-3.5" /> 数据</BaseButton
                  >
                </BaseTooltip>
                <BaseTooltip text="编辑虚拟库">
                  <BaseButton size="xs" @click="store.openEditDialog(row)"
                    ><PencilSquareIcon class="w-3.5 h-3.5" /> 编辑</BaseButton
                  >
                </BaseTooltip>
                <BaseTooltip :text="row.hidden ? '显示虚拟库' : '隐藏虚拟库'">
                  <BaseButton
                    size="xs"
                    :variant="row.hidden ? 'warning' : 'ghost'"
                    @click="store.toggleLibraryHidden(row.id)"
                  >
                    <EyeIcon
                      v-if="row.hidden"
                      class="w-3.5 h-3.5"
                ...
                <ConfirmPopover
                  :message="`确定删除虚拟库「${row.name}」？`"
                  confirm-text="删除"
                  @confirm="store.deleteLibrary(row.id)"
                >
                  <template #trigger>
                    <BaseTooltip text="删除虚拟库">
                      <BaseButton size="xs" variant="danger-outline"
                        ><TrashIcon class="w-3.5 h-3.5" /> 删除</BaseButton
                      >
                    </BaseTooltip>
                  </template>
                </ConfirmPopover>
    ...
    <div class="md:hidden space-y-3">
      ...
      >
        ...
        >
          <svg
            class="animate-spin h-8 w-8 text-primary-600 dark:text-primary-400"
            viewBox="0 0 24 24"
            fill="none"
          >
            <circle
        ...
        <div class="p-4">
          ...
          <p
            class="text-xs text-gray-500 dark:text-gray-400 truncate mb-1.5"
            :title="getResourceDetail(row)"
          >
            {{ getResourceDetail(row) }}
          </p>
          ...
          <p
            v-if="row.resource_type === 'rsshub' && row.rsshub_url"
            class="text-[11px] text-gray-400 dark:text-gray-500 break-all mb-2.5 font-mono leading-relaxed"
          >
            {{ row.rsshub_url }}
          </p>
          <!-- Action buttons -->
          <div class="flex flex-wrap gap-1.5">
            ...
            <BaseButton
              size="xs"
              :variant="row.hidden ? 'warning' : 'ghost'"
              :title="row.hidden ? '显示虚拟库' : '隐藏虚拟库'"
              @click="store.toggleLibraryHidden(row.id)"
            >
              <EyeIcon v-if="row.hidden" class="w-3.5 h-3.5" /><EyeSlashIcon
                v-else
                class="w-3.5 h-3.5"
              />
              {{ row.hidden ? "显示" : "隐藏" }}
...
const typeTagVariant = (t) =>
  ({ rsshub: "warning", random: "info", all: "primary" })[t] || "default";
...
const getSourceLibNames = (row) =>
  (row.source_libraries || []).map(getRealLibName).join(", ");

const getFilterName = (filterId) => {
  const f = (store.config.advanced_filters || []).find(
    (f) => f.id === filterId,
  );
...
const effectiveResourceIds = (row) => {
  if (Array.isArray(row.resource_ids) && row.resource_ids.length)
    return row.resource_ids.map(String);
